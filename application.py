...
# splitting imports over two lines for sake of style guide
...
def editCategory(category_name):
    ...
    if request.method == 'POST':
        # updating category
        cat_to_update = session.query(Category).filter_by(id=category.id)
        cat_to_update.update({"name": request.form['name']})
        session.commit()
